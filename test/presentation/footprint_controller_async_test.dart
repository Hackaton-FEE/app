...
void main() {
  ...
  test(
    'failed load keeps the last profile and releases the operation for retry',
    () async {
      final repository = _ControlledRepository();
      final controller = FootprintController(repository);
      addTearDown(controller.dispose);
      ...
      repository.loadGate = Completer<FootprintProfile>();
      final retry = controller.loadProfile();
  ...
  test('failed scan preserves the profile and allows a later scan', () async {
    final repository = _ControlledRepository();
    final controller = FootprintController(repository);
    addTearDown(controller.dispose);
    final initial = _profile('inicial');
    ...
    final retry = controller.scanIdentity('alias_ficticio');
    await repository.scanStarted.future;
    final scanned = _profile('alias_ficticio');
    repository.scanGate.complete(scanned);
