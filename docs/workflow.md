...
Para tareas nuevas, preferimos ramas breves desde `main`, como `feature/engineer-1-case-detail` o `fix/engineer-2-link-validation`. Las ramas `work/*` quedan disponibles para trabajo propio; no son bases de integración entre ingenieros.
...
3. Implementa y ejecuta las verificaciones aplicables. Abre un PR hacia `main` y enlaza cualquier cambio coordinado del servidor.
4. La otra persona revisa. Antes de fusionar debe existir una aprobación y checks aplicables correctos (`Flutter checks` y `Android build`). Resuelve comentarios y vuelve a verificar si cambió el código.
5. Integra mediante squash. Para la tarea siguiente, crea otra rama desde el `main` actualizado.

La aprobación y los checks son **convención, no enforcement**. En el plan GitHub Free de la organización no está disponible la protección de ramas para estos repositorios privados; una persona debe comprobar esas condiciones antes de fusionar. El build iOS se activa manualmente y requiere un runner macOS/Xcode.
...
Cada repo tiene su PR y CI. Enlaza ambos PR, explicita el contrato y conserva compatibilidad o indica el orden de integración. La revisión del cliente no sustituye la del servidor.
